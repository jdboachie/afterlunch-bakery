...
@Component({
  ...
  template: `
    <div class="px-6 py-12 h-fit flex justify-between items-center sticky top-0 z-10">
      ...
      <p class="font-serif text-xl max-sm:text-base tracking-widest max-sm:tracking-wide">
        AFTERLUNCH BAKERY
      </p>
      <div class="flex items-center relative">
        ...
        @if (cartCount > 0) {
          <span class="text-xs absolute -top-0.5 -right-0.5 bg-black text-white rounded-full size-4 grid place-items-center">{{ cartCount }}</span>
        }
      </div>
    </div>
  `,
